...
if __name__ == '__main__':
	...
	def render(reschedule=False):
		...
		with lock:
			for output, process, width in bars:
				# for debugging:
				# print(str(powerline.render(mode=modes[0], width=width, matcher_info=output).encode('utf-8') + b'\n', encoding='utf-8'))
	...
	if args.i3:
		try:
			...
			conn.on('window::title', lambda conn, evt: render())
			conn.on('window::focus', lambda conn, evt: render())
